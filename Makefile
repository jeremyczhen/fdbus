PROTO_ROOT := ../protobuf-3.0.0/build/install
PROTOC_ROOT := ../protobuf-3.0.0/protoc-tool/linux-x86_64
.PHONY: all
all:
	cwd=`pwd` && proto_root=$$cwd/${PROTO_ROOT} && \
	cmake -Bbuild \
            -Hcmake\
            -DCMAKE_INSTALL_PREFIX=/ \
	    -DSYSTEM_ROOT=$$proto_root \
	...
	cwd=`pwd` && protoc_root=$$cwd/${PROTOC_ROOT} && \
	     DESTDIR=install PATH=$$protoc_root/bin:$$PATH LD_LIBRARY_PATH=$$protoc_root/lib:$$LD_LIBRARY_PATH make -C build VERBOSE=1 -j16 install
