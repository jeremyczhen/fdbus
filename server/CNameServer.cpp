    ...
    , mHostProxy(0)
{
    ...
    mSubscribeHdl.registerCallback(NFdbBase::NTF_HOST_NAME, &CNameServer::onHostNameReg);
}
...
void CNameServer::populateList(const tAddressDescTbl &addr_tbl,
...
void CNameServer::setHostInfo(CFdbSession *session, NFdbBase::FdbMsgServiceInfo *msg_svc_info, const char *svc_name)
{
    ...
    std::string ns_url;
    CBaseSocketFactory::buildUrl(ns_url, FDB_SOCKET_TCP, host_ip.c_str(), CNsConfig::getNameServerTcpPort());
...
void CNameServer::populateServerTable(CFdbSession *session, NFdbBase::FdbMsgServiceTable &svc_tbl, bool is_local)
{
    ...
    else
    {
        for (tRegistryTbl::iterator it = mRegistryTbl.begin(); it != mRegistryTbl.end(); ++it)
        {
            ...
            populateList(it->second.mAddrTbl, *msg_svc_info->mutable_service_addr(), FDB_SOCKET_MAX);
                                    ...
                                    NFdbBase::FdbMsgAddressList *msg_addr_list)
{
    ...
    {
        desc = allocateAddress(svc_name, FDB_SOCKET_IPC);
        if (desc)
        {
    ...
    desc = allocateAddress(svc_name, FDB_SOCKET_TCP);
    if (desc)
    {
        desc->mStatus = CFdbAddressDesc::ADDR_PENDING;
        addr_tbl.mAddrTbl.push_back(desc);
                                          ...
                                          std::string &out_url)
{
    ...
    if (session_addr->mType == FDB_SOCKET_TCP)
    {
...
void CNameServer::onRegisterServiceReq(CBaseJob::Ptr &msg_ref)
{
    ...
    broadcast_ipc_addr_list.set_service_name(addr_list.service_name());
    broadcast_ipc_addr_list.set_host_name(mHostProxy->hostName());
    
    NFdbBase::FdbMsgAddressList broadcast_tcp_addr_list;
    broadcast_tcp_addr_list.set_service_name(addr_list.service_name());
    broadcast_tcp_addr_list.set_host_name(mHostProxy->hostName());

    NFdbBase::FdbMsgAddressList broadcast_all_addr_list;
    broadcast_all_addr_list.set_service_name(addr_list.service_name());
    broadcast_all_addr_list.set_host_name(mHostProxy->hostName());

    bool is_host_server = addr_list.service_name() == CNsConfig::getHostServerName();
    bool specific_ip_found = false;
            ...
            msg_it != addrs.end(); ++msg_it)
    {
        ...
        {
            if (!msg_it->compare((*addr_it)->mAddress.mUrl))
            {
                desc = *addr_it;
        ...
        if (desc)
        {
            if (desc->mAddress.mType == FDB_SOCKET_TCP)
            {
                broadcast_tcp_addr_list.add_address_list(desc->mAddress.mUrl);
                if (is_host_server && !specific_ip_found)
                {
    ...
    if (!broadcast_all_addr_list.address_list().empty())
    {
        ...
        broadcast(NFdbBase::NTF_SERVICE_ONLINE_MONITOR, broadcast_all_addr_list, svc_name);
        broadcast_all_addr_list.set_is_local(false);
        broadcast(NFdbBase::NTF_SERVICE_ONLINE_MONITOR_INTER_MACHINE, broadcast_all_addr_list, svc_name);
    }
    if (broadcast_ipc_addr_list.address_list().empty())
    {
        ...
        if (!ipc_bound)
        {
            ...
            broadcast(NFdbBase::NTF_SERVICE_ONLINE, broadcast_tcp_addr_list, svc_name);
        }
    }
    else
    {
        broadcast_ipc_addr_list.set_is_local(true);
        broadcast(NFdbBase::NTF_SERVICE_ONLINE, broadcast_ipc_addr_list, svc_name);
    }
    
    if (!broadcast_tcp_addr_list.address_list().empty())
    {
        broadcast_tcp_addr_list.set_is_local(false);
        broadcast(NFdbBase::NTF_SERVICE_ONLINE_INTER_MACHINE, broadcast_tcp_addr_list, svc_name);
    }

    checkUnconnectedAddress(addr_tbl, svc_name);
}
...
void CNameServer::checkUnconnectedAddress(CSvcRegistryEntry &desc_tbl, const char *svc_name)
{
           ...
           desc_it != desc_tbl.mAddrTbl.end(); ++desc_it)
    {
        ...
        desc->reconnect_cnt++;
        std::string url;
        if (allocateAddress(svc_name, desc->mAddress.mType, url))
        {
            // replace failed address with a new one
            if (CBaseSocketFactory::getInstance()->parseUrl(url.c_str(), desc->mAddress))
            {
...
void CNameServer::removeService(tRegistryTbl::iterator &it)
{
    const char *svc_name = it->first.c_str();
    CSvcRegistryEntry &desc_tbl = it->second;
    ...
    broadcast(NFdbBase::NTF_SERVICE_ONLINE, broadcast_addr_list, svc_name);
    broadcast(NFdbBase::NTF_SERVICE_ONLINE_MONITOR, broadcast_addr_list, svc_name);
                                        ...
                                        FdbMsgCode_t msg_code)
{
    ...
    CSvcRegistryEntry &addr_tbl = reg_it->second;
    populateList(addr_tbl.mAddrTbl, addr_list, skt_type);
    if ((skt_type == FDB_SOCKET_IPC) && addr_list.address_list().empty())
    {   /* for request from local, fallback to TCP connection */
        populateList(addr_tbl.mAddrTbl, addr_list, FDB_SOCKET_MAX);
    }
...
void CNameServer::onServiceOnlineReg(CFdbMessage *msg, const ::NFdbBase::FdbMsgSubscribeItem *sub_item)
{
    ...
    {
        const char *svc_name = service_specified ? filter : "";
        if (msg_code == NFdbBase::NTF_SERVICE_ONLINE)
        {
            mServiceSubscribedTbl.insert(svc_name);
        }
        else
        {
            mServiceMonitoredTbl.insert(svc_name);
        }
        if (name() != filter)
        {
...
void CNameServer::onHostNameReg(CFdbMessage *msg, const ::NFdbBase::FdbMsgSubscribeItem *sub_item)
{
    NFdbBase::FdbMsgServerName msg_host_name;
    msg_host_name.set_name(mHostProxy->hostName());
    msg->broadcast(sub_item->msg_code(), msg_host_name);
}
...
void CNameServer::buildUrl(const char *host_ip, const char *id, std::string &url)
{
    CBaseSocketFactory::buildUrl(url, FDB_SOCKET_TCP, host_ip, id);
}

void CNameServer::buildUrl(const char *host_ip, uint32_t id, std::string &url)
{
    CBaseSocketFactory::buildUrl(url, FDB_SOCKET_TCP, host_ip, id);
}
...
CNameServer::CFdbAddressDesc *CNameServer::createAddrDesc(const char *url)
{
    ...
    if (!CBaseSocketFactory::getInstance()->parseUrl(url, desc->mAddress))
    {
        delete desc;
        return 0;
    }
...
bool CNameServer::allocateAddress(const std::string &svc_name, EFdbSocketType type, std::string &addr_url)
{
    ...
    std::string str_url;
    const char *chr_url = 0;
    if (svc_name == CNsConfig::getHostServerName())
    {
        if (type == FDB_SOCKET_IPC)
        {
            chr_url = CNsConfig::getHostServerIpcPath();
        }
        else
        {
            // host server binds to all interface.
            buildUrl(FDB_IP_ALL_INTERFACE, CNsConfig::getHostServerTcpPort(), str_url);
            chr_url = str_url.c_str();
        }
    }
    else if (svc_name == name())
    {
        if (type == FDB_SOCKET_IPC)
        {
            chr_url = CNsConfig::getNameServerIpcPath();
        }
        else
        {
            // name server binds to all interface.
            str_url = getNsUrl(FDB_IP_ALL_INTERFACE);
            chr_url = str_url.c_str();
        }
    }
    ...
    {
        addr_url = chr_url;
        return !findAddress(type, chr_url);
    }
    ...
    for (uint32_t id = alloc_begin; id <= alloc_end; ++id)
    {
        ...
        if (type == FDB_SOCKET_TCP)
        {
            buildUrl(str_host_ip, id, url);
        }
        else
        {
            buildUrl(id, url);
        }
        if (!findAddress(type, url.c_str()))
        {
            addr_url = url;
            return true;
        }
    ...
    LOG_E("NameServer: Fatal Error! Service number out of range!\n");
    return false;
}

CNameServer::CFdbAddressDesc *CNameServer::allocateAddress(const std::string &svc_name, EFdbSocketType type)
{
    std::string url;
    return allocateAddress(svc_name, type, url) ? createAddrDesc(url.c_str()) : 0;
}
...
void CNameServer::onOffline(FdbSessionId_t sid, bool is_last)
{
    ...
    tFdbFilterSets *tbl[] = {&mServiceSubscribedTbl, &mServiceMonitoredTbl};
    FdbMsgCode_t msg_code[] = {NFdbBase::NTF_SERVICE_ONLINE, NFdbBase::NTF_SERVICE_ONLINE_MONITOR};
    for (int32_t i = 0; i < 2; ++i)
    {
        tFdbFilterSets filters;
        getSubscribeTable(msg_code[i], filters);
        for (tFdbFilterSets::iterator it = tbl[i]->begin(); it != tbl[i]->end();)
        {
            tFdbFilterSets::iterator the_it = it;
            ++it;
            if (filters.find(*the_it) == filters.end())
            {
                mHostProxy->recallServiceListener(msg_code[i], the_it->c_str());
                tbl[i]->erase(the_it);
            }
        }
    }
...
bool CNameServer::bindNsAddress(tAddressDescTbl &addr_tbl)
{
    ...
    for (tAddressDescTbl::iterator it = addr_tbl.begin(); it != addr_tbl.end(); ++it)
    {
        ...
        FdbSocketId_t sck_id = bind(url);
        if (isValidFdbId(sck_id))
        {
...
const std::string CNameServer::getNsUrl(const char *ip_addr)
{
    std::string ns_url;
    std::string host_ip;
    if (ip_addr)
    ...
    if (!host_ip.empty())
    {
        buildUrl(host_ip.c_str(), CNsConfig::getNameServerTcpPort(), ns_url);
    }
    return ns_url;
}
